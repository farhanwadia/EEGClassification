...
def runAllTrials(subject, savePath):
    trials = [["left", "open"], ["left", "closed"], ["right", "open"], ["right", "closed"]] * 5
    ...
    for r in range(numRepeats):
        for i, trial in enumerate(trials):
            description = "Trial_" + str(i) + "_" + trials[i][1] + "_" + trials[i][0]
            open_closed = trials[i][1]
            left_right = trials[i][0]
            color = 'white'
            showTrialWindow(description, open_closed, left_right, color, 8*r+i, EEGTime, EEGDelay, description, savePath, subject)
            winsound.Beep(frequency=1000, duration=500)
            showRestWindow(random.randint(4, 8))
subject = "FW"
savePath = r"C:\Users\Stephanie DiNunzio\Documents\EEG Recordings\\"
recorded_signal = []
connection = []
